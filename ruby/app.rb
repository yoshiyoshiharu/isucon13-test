      ...
      livestreams = db_transaction do |tx|
        livestream_models =
          if key_tag_name != ''
            ...
            # end
            # JOINを用いて書き直す
            tx.xquery('SELECT DISTINCT l.* FROM livestreams AS l LEFT JOIN livestream_tags AS lt ON l.id = lt.livestream_id WHERE lt.tag_id IN (?) ORDER BY l.id DESC', tag_id_list)
          else
            # 検索条件なし
