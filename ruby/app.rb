  ...
  class App < Sinatra::Base
      ...
      livestreams = db_transaction do |tx|
        livestream_models =
          if key_tag_name != ''
            ...
            tx.xquery('SELECT * FROM livestreams INNER JOIN livestream_tags ON livestreams.id = livestream_tags.livestream_id WHERE tag_id IN (?) ORDER BY livestream_id DESC', tag_id_list)
          else
            # 検索条件なし
    ...
    post '/api/livestream/:livestream_id/livecomment' do
      ...
      livecomment = db_transaction do |tx|
        ...
        # スパム判定
        ng_words = tx.xquery('SELECT id, user_id, livestream_id, word FROM ng_words WHERE user_id = ? AND livestream_id = ?', livestream_model.fetch(:user_id), livestream_model.fetch(:id))
        ng_words.each do |ng_word|
          raise HttpError.new(400, 'このコメントがスパム判定されました') if req.comment.include?(ng_word.fetch(:word))
        end
